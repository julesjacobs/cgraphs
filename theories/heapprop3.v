...
Definition heapProp_holds (P : heapProp) σ := P σ.
...
Ltac unseal := rewrite /heapProp_entails ?unseal_eqs /=.

Ltac unfold_heapProp :=
    unfold heapProp_emp_def, heapProp_pure_def, heapProp_and_def, heapProp_or_def,
      heapProp_impl_def, heapProp_forall_def, heapProp_exist_def, heapProp_sep_def,
      heapProp_wand_def, heapProp_persistently_def, heapProp_later in *.

Ltac unfolding_naive_solver := unseal; naive_solver.
...
Section mixins.
  ...
  Admitted.
    ...
    - (* [NonExpansive2 bi_and] *)
      unseal=> n P1 P2 [HP] Q1 Q2 [HQ]; split; unfolding_naive_solver.
    - (* [NonExpansive2 bi_or] *)
      unseal=> n P1 P2 [HP] Q1 Q2 [HQ]; split; unfolding_naive_solver.
    ...
    - (* [NonExpansive2 bi_persistently] *)
      unseal=> n P1 P2 [HP]; split; unfolding_naive_solver.
    - (* [φ → P ⊢ ⌜ φ ⌝] *)
      unseal=> φ P ?; by split.
    - (* [(φ → True ⊢ P) → ⌜ φ ⌝ ⊢ P] *)
    ...
    - (* [(P ⊢ Q) → (P' ⊢ Q') → P ∗ P' ⊢ Q ∗ Q'] *)
      unseal=> P P' Q Q' [HPQ] [HP'Q']; split; unfolding_naive_solver.
    - (* [P ⊢ emp ∗ P] *)
...
Proof. intros A φ. rewrite /bi_forall /bi_pure /=. unseal. done. Qed.

Opaque heapProp_entails.

Lemma foo (P Q R : heapProp) : R ∗ (P ∗ Q) -∗ (Q ∗ R) ∗ P.
Proof.
  iIntros "H".
...
Lemma disj1 (P Q : heapProp) : (P ∨ Q) ⊢ (∃ b:bool, if b then P else Q).
Proof.
  iIntros "[H|H]".
  - by iExists true.
  - by iExists false.
...
Lemma disj2 (P Q : heapProp) : (∃ b:bool, if b then P else Q) ⊢ (P ∨ Q).
Proof.
  iIntros "H".
  iDestruct "H" as ([]) "H".
  - by iLeft.
...
Lemma conj1 (P Q : heapProp) : (P ∧ Q) ⊢ (∀ b:bool, if b then P else Q).
Proof.
  iIntros "H" ([]).
  - by iDestruct "H" as "[A _]".
  - by iDestruct "H" as "[_ B]".
Qed.

Lemma conj2 (P Q : heapProp) : (∀ b:bool, if b then P else Q) ⊢ (P ∧ Q).
Proof.
  iIntros "H".
  iSplit.
  - by iSpecialize ("H" $! true).
  - by iSpecialize ("H" $! false).
Qed.

Lemma foo' (P Q R : heapProp) : R ∗ (P ∗ Q) -∗ (Q ∗ R) ∗ P.
Proof.
  iIntros "H".
  iPoseProof (foo with "H") as "G".
Qed.
